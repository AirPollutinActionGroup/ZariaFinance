...
export function DonationForm({
  ...
  const is80gEligible = donationType !== 'GIK' && !isForeign;
  const hasPanOrId = Boolean(selectedDonor?.panCardNumber || selectedDonor?.idNumber);
  ...
  const getIdNumberConfig = (type) => {
    switch (type) {
      case 'PAN':
        return { label: 'PAN card number', placeholder: 'ABCDE1234F', maxLength: 10 };
      case 'AADHAR':
        return { label: 'Aadhaar card number', placeholder: '1234 5678 9012', maxLength: 12 };
      case 'VOTER_ID':
        return { label: 'Voter ID number', placeholder: 'ABC1234567', maxLength: 10 };
      case 'DRIVING_LICENSE':
        return { label: 'Driving license number', placeholder: 'DL1420110012345', maxLength: 16 };
      case 'PASSPORT':
        return { label: 'Passport number', placeholder: 'A1234567', maxLength: 9 };
      case 'FOREIGN_TAX_ID':
        return { label: 'Foreign tax ID', placeholder: 'TAX123456', maxLength: 15 };
      default:
        return { label: 'ID number', placeholder: 'ID number', maxLength: 30 };
    }
  };
  const idNumberConfig = getIdNumberConfig(idType);
                  ...
                  {isIndividualOrMajor ? (
                    <>
                      <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                        <TextField
                          label="ID type"
                          value={selectedDonor?.idTypeLabel || selectedDonor?.idType || (selectedDonor?.panCardNumber ? 'PAN Card' : '—')}
                          disabled
                          fullWidth
                      ...
                      <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                        <TextField
                          label="ID number"
                          value={selectedDonor?.idNumber || selectedDonor?.panCardNumber || '—'}
                          disabled
                          fullWidth
                          slotProps={{ input: { sx: { textTransform: 'uppercase' } } }}
                          helperText="Derived from donor registry"
